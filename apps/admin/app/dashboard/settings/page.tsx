        ...
        <div>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-secondary p-4 md:p-8">
                ...
                <div className="pt-4 border-t border-gray-100">
                    <h3 className="text-base md:text-lg font-medium text-dark mb-3 md:mb-4">Hero Section Configuration</h3>
                    <div className="grid grid-cols-1 gap-3 md:gap-4">
                        <div>
                            <label className="block text-xs md:text-sm font-medium text-gray-700 mb-1">
                                Desktop Hero Image (720p/1080p URL)
                            </label>
                            <input
                                type="text"
                                value={settings.heroImage || ''}
                                onChange={(e) => setSettings({ ...settings, heroImage: e.target.value })}
                                placeholder="/hero-texture-wall.png or https://..."
                                className="w-full px-3 md:px-4 py-2 text-sm md:text-base border border-gray-300 rounded-md focus:ring-2 focus:ring-primary outline-none"
                            />
                            <p className="text-[10px] text-gray-500 mt-1">Leave empty to use default texture.</p>
                        </div>
                        <div>
                            <label className="block text-xs md:text-sm font-medium text-gray-700 mb-1">
                                Mobile Hero Image (Portrait URL)
                            </label>
                            <input
                                type="text"
                                value={settings.heroImageMobile || ''}
                                onChange={(e) => setSettings({ ...settings, heroImageMobile: e.target.value })}
                                placeholder="/hero-mobile-v2.png or https://..."
                                className="w-full px-3 md:px-4 py-2 text-sm md:text-base border border-gray-300 rounded-md focus:ring-2 focus:ring-primary outline-none"
                            />
                        </div>
                    </div>
