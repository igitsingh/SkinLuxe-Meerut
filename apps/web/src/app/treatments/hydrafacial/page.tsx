...
export default function HydraFacialPage() {
    const [price, setPrice] = useState("₹3,999"); // Fallback
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        const fetchPrice = async () => {
            try {
                const res = await api.get('/treatments/hydrafacial');
                if (res.data && res.data.price) {
                    // Extract price if it has text like "Starts at" or just use as is
                    // Admin input: "Starts at ₹4,999"
                    // We can display the full string or try to parse.
                    // Let's display the full string for accuracy with Admin.
            ...
            <section className="relative min-h-[75vh] flex items-center justify-center bg-[#1C1C1C] text-white pt-32 pb-20 overflow-hidden">
                ...
                <div className="container relative z-10 text-center max-w-5xl">
                    ...
                    <h1 className="font-serif text-5xl md:text-7xl lg:text-8xl mb-6 leading-tight text-white animate-fadeIn delay-100">
                        ...
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-white via-[#B4838D] to-white italic inline-block pr-2">
